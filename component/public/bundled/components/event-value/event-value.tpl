	...
	<component :is="inline ? 'div' : 'n-collapsible'" :title="title" class="page-event-value">
		<div :class="{'padded-content': !inline}">
			...
			<div v-for="eventField in container[name].eventFields" class="list-row">
				...
				<span @click="container[name].eventFields.splice(container[name].eventFields.indexOf(eventField), 1) & $updateEvents()" class="fa fa-times"></span>
			</div>
		</div>
		<div class="list-actions" v-if="container[name].name">
			<button @click="addEventField(container[name]) & $updateEvents()"><span class="fa fa-plus"></span>Event Field</button>
		</div>
	</component>
</template>
